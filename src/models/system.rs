...
mod test {
    ...
    fn test_get_system() {
        ...
        assert_eq!(system.data.symbol, SystemSymbol("X1-HN18".into()));
    }
    ...
    fn test_get_waypoints() {
        ...
        let waypoints: PaginatedList<Waypoint> = serde_json::from_str(waypoint_json).unwrap();
        assert_eq!(
            waypoints.data[0].symbol,
            WaypointSymbol("X1-HN18-ZX1B".into())
        );
    ...
    fn test_system_dot_json() {
        ...
        assert_eq!(systems[0].symbol, SystemSymbol("X1-HN18".into()));
    }
}
